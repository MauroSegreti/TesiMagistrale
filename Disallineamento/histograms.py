...
def print_window_summary():
    print("\n=== Semi-larghezza della finestra per bin di pT x eta ===")
    header = f"{'bin pT':>12} {'sigma att.':>11}"
    for e in ETA_BINS:
        header += f"  {'%.1f-%.1f' % (e['min'], e['max']):>10}"
    print(header)
    print(f"{'':>12} {'scale ->':>11}" +
          "".join(f"  {s:>10.1f}" for s in ETA_WINDOW_SCALE))
    for p in PT_BINS:
        s = expected_sigma(p["mean"])
        row = f"{p['name']:>12} {s:11.4f}"
        for scale in ETA_WINDOW_SCALE:
            row += f"  {window_for(p['mean'], scale):>10.3f}"
        print(row)
    print()

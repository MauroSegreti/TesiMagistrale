...
# job leggono da remoto. Cosi' la statistica non dipende da quali file erano
# stati scaricati in locale -- il download della Z si era fermato a 30 file
# su 68, e per un po' abbiamo girato con meta' campione senza accorgercene.
#
# Uso:  ./gen_jobs.sh
# Poi:  condor_submit condorSub.sub

ANADIR=$(cd "$(dirname "$0")" && pwd)
OUTDIR=/eos/user/m/masegret/risoluzione_quantili_out

# Liste dei sample ad allineamento perfetto: Z (601190) + 5 masse di Z'.
# NON usare MisAligned_MC/ ne' Nominal_MC/: sono altre condizioni.
LISTS=/afs/cern.ch/user/m/masegret/PerfectAlignment
